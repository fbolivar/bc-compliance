      ...
      <div className="md:hidden space-y-2">
        {filteredData.length === 0 ? (
          ...
          filteredData.map((item, idx) => {
            // Columns visible on mobile = not hideOnMobile, sorted by priority
            const mobileColumns = columns
              .filter(c => !c.hideOnMobile)
              .sort((a, b) => (a.priority ?? 99) - (b.priority ?? 99));
            const primaryCol = mobileColumns[0];
            const secondaryCol = mobileColumns[1];
            const detailCols = mobileColumns.slice(2, 5);
            const hiddenCount = columns.length - mobileColumns.length + Math.max(0, mobileColumns.length - 5);
            return (
              <div
                key={String(item[idField]) || idx}
                className="bg-white border border-slate-200 border-l-[3px] border-l-sky-400 rounded-xl shadow-sm overflow-hidden"
              >
                {/* Card header row */}
                  ...
                  <div className="min-w-0">
                    ...
                    {secondaryCol && (
                      <div className="text-xs text-slate-500 mt-0.5 truncate">
                        {secondaryCol.render ? secondaryCol.render(item) : String(item[secondaryCol.key] ?? '-')}
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="px-4 pb-2 space-y-1.5">
                    {detailCols.map(col => (
                      <div key={col.key} className="flex items-center justify-between gap-3">
                        <span className="text-xs text-slate-400 shrink-0 w-24">{col.label}</span>
                    ...
                    {hiddenCount > 0 && (
                      <p className="text-[10px] text-slate-400 text-right pt-0.5">{hiddenCount} campo{hiddenCount !== 1 ? 's' : ''} más al ver detalle</p>
                    )}
                  </div>
                )}

                {/* Actions */}
                <div className="flex items-center justify-end gap-1 px-3 py-2 border-t border-slate-100 bg-slate-50/70">
                  <Link
                  ...
                  {onDelete && (
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-rose-500 hover:bg-rose-50 rounded-lg transition-colors"
                    >
