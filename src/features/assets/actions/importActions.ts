...
export interface ImportResult {
  ok: boolean;
  inserted?: number;
  /** Rows skipped because the code already exists in the DB (duplicates). */
  skippedDuplicates?: number;
  /** Rows rejected with a validation error (missing code, DB insert error, etc). */
  rejected?: number;
  ...
  diagnostic?: {
    ...
    /** Rows whose "Proceso" column matched a seeded process (category_id assigned). */
    processMatched?: number;
    /** Rows that fell back to the calling page's process (no match or blank). */
...
/** Strips the short-code prefix ("E1 · ", "M2 · ", "A3 · ", etc.) and normalizes. */
...
const FIELD_ALIASES: Record<string, string[]> = {
  ...
  trd_serie: ['trd_serie_sub_serie', 'trd_serie_subserie', 'trd', 'trd_serie', 'serie_documental'],
  name: [
  ...
  description: ['descripcion_del_activo', 'descripcion_de_activo', 'descripcion', 'description'],
  ...
  support: [
    ...
    'soporte_medio_de_conservacion_consulta', 'medio_de_conservacion_consulta',
  ],
  consultation_place: [
  ...
  icc_is_critical: ['activo_icc', 'icc_critico', 'es_icc', 'activo_de_icc'],
  ...
  exception_objective: [
    'objetivo_excepcion', 'objetivo_de_excepcion', 'objetivo_legitimo_de_la_excepcion',
  ],
  constitutional_basis: ['fundamento_constitucional', 'fundamento_constitucional_o_legal'],
  legal_exception_basis: [
    'fundamento_juridico', 'fundamento_legal_excepcion',
    'fundamento_juridico_de_la_excepcion',
  ],
  exception_scope: ['excepcion_total_parcial', 'alcance_excepcion', 'excepcion_total_o_parcial'],
  classification_date: [
    'fecha_calificacion', 'fecha_clasificacion',
    'fecha_de_la_calificacion', 'fecha_de_la_calificacion_dd_mm_aaaa',
  ],
  classification_term: ['plazo_reserva', 'termino_reserva', 'plazo_de_clasificacion_o_reserva'],
...
function detectHeaders(sheet: ExcelJS.Worksheet): {
  ...
  for (let r = 1; r <= maxScan; r++) {
    ...
    row.eachCell({ includeEmpty: false }, (cell, colNumber) => {
      ...
      if (!raw) return;
      if (raw.trim().endsWith(':')) return; // skip metadata labels
      const normalized = normalizeHeader(raw);
      if (!normalized) return;
      if (seenNormalized.has(normalized)) return; // dedupe merged-cell repeats
...
export async function importAssets(formData: FormData): Promise<ImportResult> {
  try {
    ...
    if (!detected) {
      ...
      return {
        ok: false,
        error: 'No se pudo detectar la cabecera. El archivo necesita columnas reconocibles como "Código" y "Nombre del Activo".',
        diagnostic: {
    ...
    // Pre-build the process name → category_id lookup for this org.
    ...
    for (const cat of (processCats ?? []) as Array<{ id: string; name: string }>) {
      processByName.set(normalizeProcessName(cat.name), cat.id);
    }

    const errors: ImportResult['errors'] = [];
    const records: Array<{ record: Record<string, unknown>; row: number; fallbackUsed: boolean }> = [];
    let rowsScanned = 0;
    ...
    for (let r = dataStartRow; r <= lastRow; r++) {
      ...
      // Resolve category_id from the row's Proceso column, falling back to the
      // process the user was on when they clicked Importar.
      const processNameRaw = readCell(row, columnMap, 'process_name');
      const matchedId = processNameRaw ? processByName.get(normalizeProcessName(processNameRaw)) : undefined;
      const categoryId = matchedId ?? fallbackProcessId ?? null;
      const fallbackUsed = !matchedId && !!fallbackProcessId;
      ...
      const record: Record<string, unknown> = {
        ...
        process_name: processNameRaw || null,
      ...
      records.push({ record, row: r, fallbackUsed });
    }

    // Count process matching stats for diagnostic
    const processMatched = records.filter((e) => !e.fallbackUsed).length;
    const processFallback = records.filter((e) => e.fallbackUsed).length;
    ...
    // Skip duplicates: fetch existing codes in one query
    const incomingCodes = records.map((e) => e.record.code as string);
    const { data: existingRows } = await supabase
      .from('assets')
      .select('code')
      .eq('organization_id', orgId)
      .in('code', incomingCodes);
    const existingSet = new Set((existingRows ?? []).map((r) => (r as { code: string }).code));
    ...
    for (const entry of records) {
      const code = entry.record.code as string;
      if (existingSet.has(code)) {
        skippedDuplicates++;
        continue;
      }

      const { error } = await supabase.from('assets').insert(entry.record);
      if (error) {
        rejected++;
        errors.push({ row: entry.row, code, message: error.message });
      } else {
        inserted++;
      }
    }
    ...
    await writeAuditLog({
      action: 'create',
      tableName: 'assets',
      description: `Importación masiva: ${inserted} nuevos, ${skippedDuplicates} duplicados omitidos, ${rejected} rechazados`,
    });
