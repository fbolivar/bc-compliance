...
export default async function RiskMatrixPage() {
  ...
  const levels = [
    ...
    { key: 'negligible', label: 'Despreciable', color: 'bg-slate-500', textColor: 'text-slate-400', barColor: 'bg-slate-500' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-5 gap-4">
        {levels.map(l => (
          <div key={l.key} className="bg-slate-900/50 border border-slate-800 rounded-xl p-6 text-center">
      ...
      <div className="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider mb-4">Distribucion</h3>
        <div className="space-y-3">
          {levels.map(l => (
            <div key={l.key} className="flex items-center gap-4">
              <div className={`w-3 h-3 rounded-full ${l.color}`} />
              <span className="text-sm text-slate-400 w-28">{l.label}</span>
