      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {REPORT_TEMPLATES.map((template) => (
            <div
              key={template.id}
              className="rounded-xl border border-slate-200 bg-white p-5 hover:border-slate-300 transition-colors group shadow-sm"
            >
              <div className="flex items-start justify-between mb-3">
                <span className="text-2xl">{template.icon}</span>
                <span className="px-2 py-0.5 text-xs bg-slate-100 text-slate-500 border border-slate-200 rounded font-mono">
                  {template.format}
                </span>
              </div>
              <h3 className="text-sm font-semibold text-slate-700 mb-1">{template.name}</h3>
              <p className="text-xs text-slate-500 mb-4">{template.description}</p>
              <button
                disabled
                className="w-full flex items-center justify-center gap-2 px-3 py-2 text-xs text-slate-400 border border-slate-200 rounded-lg cursor-not-allowed"
                title="Proximamente"
              >
          ...
          <div className="rounded-xl border border-slate-200 bg-white overflow-hidden shadow-sm">
            <div className="divide-y divide-slate-100">
              {completedAudits.data.map((audit) => (
                <div key={audit.id} className="px-4 py-4 hover:bg-slate-50 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      {audit.report_url ? (
                        ...
                        <Link
                          ...
                          className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-slate-500 border border-slate-200 rounded-lg hover:border-slate-300 transition-colors"
                        >
                          Ver Auditoria
                        </Link>
                      )}
